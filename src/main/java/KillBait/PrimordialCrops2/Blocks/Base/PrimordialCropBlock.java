...
public class PrimordialCropBlock extends BlockCrops implements IGrowable, IPlantable, TOPInfoProvider, WailaInfoProvider {
	...
	// MUST overwrite this for each class to get the correct drops
	@Override
	protected Item getSeed() { return Items.WHEAT_SEEDS; }

	// MUST overwrite this for each class to get the correct drops
	@Override
	protected Item getCrop() { return Items.WHEAT; }
	...
	public List<ItemStack> getDrops(IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
		...
		if (getAge(state) >= 3) {
			...
			ret.add(new ItemStack(this.getCrop(), getFarmlandBonus(world, pos),0));
	...
	protected int getBonemealAgeIncrease(World worldIn) {
		return super.getBonemealAgeIncrease(worldIn) / 4; // divide by number >= maxage x2 means bonemeal doesnt work
	}
	...
	public int getFarmlandBonus(IBlockAccess world, BlockPos pos) {
		...
		if (checkblock instanceof InfusedDirtBlock) {
			//LogHelper.info(getblock);
			int meta = checkblock.getMetaFromState(world.getBlockState(pos.down()));
			//LogHelper.info(meta);
			if (meta == 0) {
			...
			LogHelper.error("Invalid Metadata " + meta + " for " + checkblock);
		}
		return 1;
	}
